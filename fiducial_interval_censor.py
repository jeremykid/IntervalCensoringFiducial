...
# Try to import OSQP for fast QP solving
try:
    import osqp
...
# Numerical constants
_EPS = 1e-10  # Small epsilon for numerical stability
...
def _linear_interpolation_qp(
    ...
    if solver == "osqp" and OSQP_AVAILABLE:
        try:
            ...
            warnings.warn(f"OSQP failed ({e}), falling back to scipy")
...
def fit_fiducial_interval_censor(
    ...
    # Determine solver
    if solver == "osqp" and not OSQP_AVAILABLE:
        warnings.warn("OSQP not available, falling back to scipy solver")
        solver = "scipy"
...
def _validate_results(
    ...
    """
    # (a) Check [0, 1] bounds
    if np.any(FiducialMidLine < -1e-10) or np.any(FiducialMidLine > 1 + 1e-10):
    ...
    if np.any(F_low > F_median + 1e-10):
        warnings.warn("F_low > F_median detected")
    if np.any(F_median > F_high + 1e-10):
...
def _demo():
    ...
    result = fit_fiducial_interval_censor(
        ...
        ntest=200,
        seed=123,
        solver="osqp" if OSQP_AVAILABLE else "scipy"
    )
    ...
    F_monotonic = np.all(np.diff(result['F_median']) >= -1e-6)
